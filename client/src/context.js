...
const Context = ({children}) => {
  ...
  const [text, setTextToAdd] = useState("");
  ...
  const [viewOwnDrawing, setViewOwnDrawing] = useState(false);
  ...
  const addTextToCanvas = useCallback((e) => {
    if (brushMode === "text" && (brush || role === "leader")) {
      const x = e.offsetX;
      const y = e.offsetY;
      ctx.fillStyle = brushColor;
      ctx.font = `${brushWidth}px Arial`; // Définir la taille de la police
      ctx.fillText(text, x, y);
      // Émettre le texte ajouté via socket si nécessaire
    }
  }, [brushMode, brush, role, brushColor, brushWidth, ctx, text]);
  ...
  return (
    <AppContext.Provider value={{
      ...
      changeBgColor,
      handleClearCanvas,
      viewOwnDrawing, // Passer l'état pour voir son propre dessin
      setViewOwnDrawing // Passer la fonction pour modifier l'état de voir son propre dessin
    }}>
      {children}
    </AppContext.Provider>
  )
